        ...
        <div className="mt-8 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {generatedNames.map((name, index) => (
              <div
                key={index}
                className="glass-effect p-6 rounded-lg flex justify-between items-center animate-fade-in hover:bg-white/10 transition-all duration-300 group relative overflow-hidden border border-white/10 shadow-lg"
                style={{ 
              ...
              >
                ...
                <span className="font-medium text-lg relative z-10 text-white group-hover:text-primary transition-colors duration-300">{name}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => copyToClipboard(name)}
                  className="relative z-10 hover:bg-primary/20 transition-colors ml-4"
                >
